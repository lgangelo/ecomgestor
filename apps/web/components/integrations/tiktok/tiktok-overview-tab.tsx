...
import { ShoppingBag, Webhook, Wallet, AlertTriangle } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { StatusBadge } from '@/components/ui/status-badge';
import { TIKTOK_AREA_STATUS_PRESENTATION } from '@ecommerce-manager/ui';
...
export function TikTokOverviewTab({ status }: { status: TikTokStatus }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle>Pedidos com SKU pendente</CardTitle>
          </CardHeader>
          <CardContent className="pt-0 text-2xl font-semibold">{status.pendingMappingCount ?? 0}</CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle>Webhooks recebidos (24h)</CardTitle>
            <Webhook className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent className="pt-0 text-2xl font-semibold">{status.last24h?.webhooksReceived ?? 0}</CardContent>
        </Card>
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-2 pt-0 text-sm sm:grid-cols-3">
          <div>
          ...
          <div>
            ...
            {timeAgo(status.checkpoints?.financeSyncAt)}
          </div>
        </CardContent>
      </Card>
    </div>
