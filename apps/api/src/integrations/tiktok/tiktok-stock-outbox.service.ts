...
export class TikTokStockOutboxService {
  ...
  /**
   * Só envia de fato se a flag global (`TIKTOK_INVENTORY_PUSH_ENABLED`) E o toggle por empresa
   * (`Company.inventoryAutoSyncEnabled`) estiverem ligados (seção 56) — os dois continuam
  ...
  async processPending(companyId: string): Promise<{ processed: number; failed: number }> {
    ...
    const company = await this.prisma.client.company.findUnique({
      where: { id: companyId },
      select: { inventoryAutoSyncEnabled: true },
    });
    if (!company?.inventoryAutoSyncEnabled) return { processed: 0, failed: 0 };
